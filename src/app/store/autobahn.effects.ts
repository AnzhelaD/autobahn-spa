...
export class AutobahnEffects {
  ...
  loadRoadworks$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((action) => {
        return this.autobahnService
          .getRoadworks(action.id)
          .pipe(map((data) => setData({ data: data?.roadworks })));
      })
    )
  );
  ...
  loadWebCams$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((action) => {
        return this.autobahnService
          .getWebCams(action.id)
          .pipe(map((data) => setData({ data: data?.webcam })));
      })
    )
  );
